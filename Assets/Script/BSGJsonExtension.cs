...
public static class BSGJsonExtension
{
    ...
    /// <returns></returns>
    public static T BSGReadJson<T>(string file, T defaultInfo = default(T), System.Action fileNotFoundException = null)  
    {
        ...
        else
        {
            ...
            s = defaultInfo;
            //创建json文件
            BSGCreatJson<T>(file, s);
            return s;
        }
    }


    public static bool BSGSaveJson<T>(string file, T info)
    {
        var url = Application.persistentDataPath + "/" + file;
    ...
    //创建json文件
    private static T BSGCreatJson<T>(string file, T defaultInfo)
    {
        var url = Application.persistentDataPath + "/" + file;
